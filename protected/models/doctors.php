...
class doctors extends BaseModel
{
    ...
    public function relations()
    {
        return array(
			...
			'comments' => array(self::HAS_MANY, 'Comments', 'object_id', 'condition' => 'comments.object_type = '.Objects::model() -> getNumber(get_class($this)). ' AND comments.approved = 1'),
			...
			'employments' => array(self::HAS_MANY, 'Employment', 'id_doctor')
        );
    }
	...
	*/
	...
	public function giveAddressArray() {
		return array_filter(array_map(function ($cl) { return $cl -> giveAddress(); }, $this -> clinics));
	}
	public function giveAddressString($del = ' ,') {
		return implode($del,$this -> giveAddressArray());
	}
	public function giveCoordsArray(){
		$rez = array();
		foreach ($this -> clinics as $cl) {
			if ($coords = $cl -> giveCoords()) {
				$rez [$cl -> address] = $coords;
			}
		}
		return $rez;
	}
