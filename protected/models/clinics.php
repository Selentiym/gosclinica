...
class clinics extends BaseModel
{
	...
	public function rules()
	{
		return array(
			...
			array('doctorsInput, about_clinic, prices_text, useDocDoc', 'safe')
		);
	}
	...
	*/
	...
	public function giveCoords() {
		if (!$this -> map_coordinates) {
			$adress=urlencode($this -> address);
			...
			if (trim(next($found)) > 0) {
				...
				new CustomFlash('warning','Clinics','coords','Координаты адреса '.$address.' не определены',true);
			}
		}
