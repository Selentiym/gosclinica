...
class BaseModel extends CTModel
{
	...
	public function userSearch($search,$order='rating',$limit=-1)
	{
		...
		if ($order == 'price') {
			usort($objects_filtered, function ($obj1, $obj2) {
				$pr1 = (current($obj1 -> prices));
				$pr2 = (current($obj2 -> prices));
				if ((!$pr1) && (!$pr2)) {return true;}
				if ((!$pr1)) {return true;}
				if (!$pr2) {return false;}
				//echo $pr1 -> price - ($pr2 -> price)*1;echo"<br/>";
				return (($pr1 -> price) - ($pr2 -> price) > 0);
			});
		}
		//За одно возвращаем модель описания к заданному поиску.
		$rez['objects'] = $objects_filtered;
		//giveModelByTriggerArray
		$rez['description'] = Articles::model() -> giveModelByTriggerArray($filter);
	...
	*/
	...
	public function beforeDelete() {
		parent::beforeDelete();
		$del = true;
		if (in_array(get_class($this),array('clinics','doctors'))) {
			//delete the reviews
			foreach ($this -> comments as $comm) {
				if (!$comm -> delete()) {
					$del = false;
					print_r($empl -> getErrors());
				}
			}
			//delete the employments
			//var_dump($this -> employments );
			$criteria = new CDbCriteria;
			$field = get_class($this) == "clinics" ? "id_clinic" : "id_doctor";
			$criteria -> compare($field,$this -> id);
			$empls = Employment::model() -> findAll($criteria);
