...
class AdminController extends Controller
{
    ...
    public function actionModels($modelName)
	{
        ...
        if(isset($_GET[(strtolower($modelName))]))
            $model->attributes = $_GET[(strtolower($modelName))];
		/*ob_start();
		//var_dump($_REQUEST);
		echo "<br/>";
		var_dump($_GET[(strtolower($modelName))]);
		echo "<br/>";
		//var_dump($_POST);
		$out = ob_get_contents();
		ob_end_clean();
		...
		fwrite($f, 'There was a request <pre>'.date('j.m H:i:s').':<br/>'.PHP_EOL);
		fwrite($f, $out.'</pre><br/><br/>'.PHP_EOL);
		fclose($f);*/
	...
	public function actionViewLog(){
		$dateStr = date("_Y_M_d");
		echo file_get_contents(Yii::getPathOfAlias('application'). DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . "log{$dateStr}.txt");
	}
    ...
    public function actioncommentToggle(){
		$comment = Comments::model()->findByPk($_GET['id']);
		if ($comment->approved == 1)
           $comment->approved = 0;
        else
           $comment->approved = 1;
	   if ($comment->save()) {
		   $this -> redirect(Yii::app() -> baseUrl.'/admin/comments');
	   }
	}
	/*public function actioncommentToggle()
    {
	...
	public function actionImportCsv($modelName)
	{
		...
		if ($handle)
		{
			...
			$modelName::model() -> ImportCsv($handle, $this -> $attrName);
			fclose($handle);
		}
